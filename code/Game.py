...
class boss():
  def __init__(self, x_boss, y_boss, x_size, y_size):
...
def game_loop_3():
  ...
  for i in range(len(bosses_3)):
    if ((B.x - bosses_3[i].x)**2 + (B.y - bosses_3[i].y)**2)**0.5 < bosses_3[i].y_size and time.time() - T2 > 1 :
      enemy_lives.decrease(20)
      if bosses_3[i].y_size != 8:
        bosses_3[i].y_size -= 4
        bosses_3[i].y_size -= 10
        bosses_3[i].y_direction = 0
        bosses_3[i].vx += 1
        bosses_3[i].vy += 1
        bosses_3.append(boss(bosses_3[i].x, bosses_3[i].y + 10, bosses_3[i].x_size, bosses_3[i].y_size))
        n = len(bosses_3)
        bosses_3[n-1].vx += 1
        bosses_3[n-1].vy += 1
      else:
  ...
  for i in range(len(enemy_bullet_3)):
    t.append(T5)
  for i in range(len(enemy_bullet_3)):
    if ((enemy_bullet_3[i].x - x)**2 + (enemy_bullet_3[i].y - y)**2)**0.5 < 10 and time.time() - t[i] > 1 :
...
if level == 1:
  ...
  img_boss = img_boss.resize((100, 150), Image.ANTIALIAS)
  img_boss = ImageTk.PhotoImage(img_boss)
  img_mini_boss = Image.open('boss_1.jpg')
  img_mini_boss = img_mini_boss.resize((50, 75), Image.ANTIALIAS)
  img_mini_boss = ImageTk.PhotoImage(img_mini_boss)
  boss_1 = boss(500, 500, 50, 75)
  enemy_bullet = []
...
if level == 2:
  ...
  img_boss = img_boss.resize((100, 100), Image.ANTIALIAS)
  img_boss = ImageTk.PhotoImage(img_boss)
  bosses_2.append(boss(500, 500, 50, 50))
  bosses_2.append(boss(300, 300, 50, 50))
  bosses_2.append(boss(500, 100, 50, 50))
...
if level == 3:
  bosses_3 = []
  bosses_3.append(boss(500, 500, 20))
  enemy_lives = health_indicator(299, 420, 570, 'red')
  enemy_bullet_3 = []

...
def game_loop_3():
  ...
  for i in range(len(bosses_3)):
    if ((B.x - bosses_3[i].x)**2 + (B.y - bosses_3[i].y)**2)**0.5 < bosses_3[i].r and time.time() - T2 > 1 :
      ...
      if bosses_3[i].r != 8:
        ...
        bosses_3[i].vx += 1
        bosses_3[i].vy += 1
        bosses_3.append(boss(bosses_3[i].x, bosses_3[i].y + 10, 'medium blue'))
        n = len(bosses_3)
        bosses_3[n-1].r = bosses_3[i].r
        bosses_3[n-1].vx += 1
        bosses_3[n-1].vy += 1
      else:
...
if level == 3:
  bosses_3 = []
  bosses_3.append(boss(500, 500, 'medium blue'))
  enemy_lives = health_indicator(299, 420, 570, 'red')
  enemy_bullet_3 = []

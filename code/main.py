...
class mini_twins(boss):
    def __init__(self, x_mini, y_mini):
        ...
        self.x_direction = 1
        self.y_direction = 1
        self.id = canvas.create_image(self.x - int(self.x_size / 2), self.y - int(self.x_size / 2), image=img_mini_boss,
                                      anchor='nw')
...
def hit_character(array_bullet, T, strip, d):
    ...
    for i in range(len(array_bullet)):
        t.append(T)
    for i in range(len(array_bullet)):
        if ((array_bullet[i].x - x) ** 2 + (array_bullet[i].y - y) ** 2) ** 0.5 < character_size / 3 and time.time() - \
                t[i] > 1:
            strip.decrease(d)
            t[i] = time.time()
...
def hit_enemy(array_enemyies, array_strips, d):
    ...
    for i in range(len(array_enemyies)):
        if ((B.x - array_enemyies[i].x) ** 2 + (B.y - array_enemyies[i].y) ** 2) ** 0.5 < array_enemyies[
            i].y_size and time.time() - T2 > 1:
...
def level_1():
    """Главная функция первого уровня
      ...
      Бой происходит по принципу перестрелки: спустя некоторое регулируемое время противник и все его копии одновременн стреляют точно в игрока, поэтому чтобы успеть среагировать необходимо постоянно деражаться далеко от противника и его копий, стараться их уничтожать.
    """
    ...
    global T4
    if time.time() - T4 > 5 and len(twins) != 0:
...
def level_2():
    ...
    hit_enemy(bosses_2, enemy_lives, 10)
...
def level_3():
    """Главная функция третьего уровня
      На третьем уровне противник при каждом попадании в него делится на два новых меньшего размера и большей скорости, в которых труднее попасть. Деление происходит три раза и в конечном счете крупный противник последоательно распадается на 16 мелких, каждый из оставшихся 16 при попадании уничтожается
    """
    global x, y
    x += vx
    y += vy
    ...
    for i in range(len(bosses_3)):
        if ((B.x - bosses_3[i].x) ** 2 + (B.y - bosses_3[i].y) ** 2) ** 0.5 < bosses_3[
            i].y_size / 2 and time.time() - T2 > 1:
            enemy_lives.decrease(20)
            if bosses_3[i].y_size != boss_3_x_size - 3 * delta:
                bosses_3[i].c += 1
                bosses_3[i].y_size -= delta
                bosses_3[i].x_size -= delta
                bosses_3.append(boss(bosses_3[i].x, bosses_3[i].y + delta / 2, bosses_3[i].x_size, bosses_3[i].y_size,
    ...
    explosion(randint(10, 15), 'cyan', enemy_bullet_3, bosses_3, 1)

...
while True:
    ...
    def move_location(loc_name) :
        ...
        t = time.time()
        char_x = 0
        character = canvas.create_image(char_x, 400, image=img_character_1, anchor='nw')
        while char_x < 600:
    ...
    def hit_character(array_bullet, T, strip, d):
        ...
        for i in range(len(array_bullet)):
            t.append(T)
        for i in range(len(array_bullet)):
            if ((array_bullet[i].x - x) ** 2 + (
                    array_bullet[i].y - y) ** 2) ** 0.5 < character_size / 3 and time.time() - t[i] > 1:
                strip.decrease(d)
                t[i] = time.time()
    ...
    def level_3():
        ...
        explosion(randint(20, 30), 'cyan', enemy_bullet_3, bosses_3, 1)
